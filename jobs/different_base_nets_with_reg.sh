...
source startup.sh
python model.py -e 120 -d '../datasets' -m ${OUTPUT_FOLDER}/${FILE_NAME}.model  --base-net=${NET} --base-net-args regularization_type=${REGULARIZATION_TYPE} regularization_rate=${REGULARIZATION_RATE} -f ${AGGREGATE_FEATURE_COUNT} --non-linear=${NON_LINEAR} > ${OUTPUT_FOLDER}/outs/${FILE_NAME}.out
